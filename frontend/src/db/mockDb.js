// Mock Database for AI-Powered Smart HRMS
// Handles loading and saving data to localStorage to ensure persistence.
...
const defaultAttendance = [
  // Prepopulate yesterday's attendance for employees
...
// Database operations with localStorage
export const db = {
  init() {
    ...
    if (!localStorage.getItem(STORAGE_KEYS.POLICIES)) {
      localStorage.setItem(STORAGE_KEYS.POLICIES, JSON.stringify(defaultPolicies));
    }
    if (!localStorage.getItem(STORAGE_KEYS.SETTINGS)) {
      localStorage.setItem(STORAGE_KEYS.SETTINGS, JSON.stringify({ geminiKey: '' }));
    }
  },
  ...
  checkIn(employeeId) {
    ...
    list.unshift(newEntry); // Put newest first
  ...
  getGeminiKey() {
    ...
    return settings ? settings.geminiKey : '';
  },
  saveGeminiKey(key) {
    const settings = this.getSettings() || {};
